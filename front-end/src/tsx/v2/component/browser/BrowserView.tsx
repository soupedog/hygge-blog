...
function renderMainImage(currentArticle: ArticleDto | null) {
    if (currentArticle != null) {
        return (
            <div style={{
                ...
                background: "url(" + currentArticle.imageSrc + ") no-repeat center"
            }}/>
        );
    }
}

function renderMusicPlayer(currentArticle: ArticleDto | null) {
    if (currentArticle != null) {
        return (
            <MusicPlayer configuration={currentArticle.configuration}/>
        );
    }
}
                ...
                <Content id="mainView">
                    <Card title={currentArticle.title} bordered={false}
                        ...
                        <div style={{
                            ...
                            <Space size={"middle"}>
                                <IconText icon={EditTwoTone} text={"字数 " + currentArticle.wordCount}
                                          key={"word_count_" + currentArticle.aid}/>
                                <IconText icon={DashboardTwoTone}
                                          text={"创建于 " + TimeHelper.formatTimeStampToString(currentArticle.createTs)}
                                          key={"create_ts_" + currentArticle.aid}/>
                                <IconText icon={DashboardTwoTone}
                                          text={"最后修改于 " + TimeHelper.formatTimeStampToString(currentArticle.lastUpdateTs)}
                                          key={"lastUpdate_ts_" + currentArticle.aid}/>
                                <IconText icon={EyeTwoTone} text={"浏览量 " + currentArticle.pageViews}
                                          key={"page_view_" + currentArticle.aid}/>
                                {isAuthor ? <IconText icon={EyeOutlined}
                                                      text={"自浏览 " + currentArticle.selfPageViews}
                                                      key={"self_view_" + currentArticle.aid}/> : null}
                            </Space>
                        </div>
                    </Card>
