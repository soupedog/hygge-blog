...
public interface FileControllerDoc extends HyggeBlogController<ResponseEntity<?>> {
    ...
    ResponseEntity<HyggeBlogControllerResponse<List<FileInfoDto>>> upload(@Parameter FileTypeEnum fileType, @Parameter Integer permissionId, @Parameter(description = "文章类别唯一标识展示用编号") String cid, List<MultipartFile> filesList);
